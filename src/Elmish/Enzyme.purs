...
-- | always a "current" DOM element (or, more precisely, current `Wrapper`,
-- | which may refer to zero or more actual DOM elements), with respect to which
-- | all operations run. A test starts with either `testComponent` or
-- | `testElement`, and it's that component (or element) that becomes the
-- | "current" element at the start, for example:
-- |
-- |```purs
-- |-- The main module:
...
-- |```
-- |
-- | Alternatively, operations that return a `ChildWrapper` may be combined in a
-- | chain using the `>>` operator (which is just a convenient facade for
...
import Elmish.Enzyme.Foreign (class Wrapper, ChildWrapper, RootWrapper)
import Elmish.Enzyme.Foreign as E
import Foreign (Foreign)

-- | Monad for running Enzyme tests. Keeps a reference to the "current" DOM
-- | element(s).
type EnzymeM wrapper = ReaderT wrapper Aff
...
-- | ```
testComponent :: forall m msg state. MonadAff m => ComponentDef msg state -> EnzymeM RootWrapper Unit -> m Unit
...
-- |     exists ".t--my-content" >>= shouldEqual true
-- | ```
testElement :: forall m. MonadAff m => ReactElement -> EnzymeM RootWrapper Unit -> m Unit
...
-- |```
at :: forall wrapper. Wrapper wrapper => Int -> EnzymeM wrapper ChildWrapper
...
-- | info.
debug :: forall wrapper. DebugWarning => Wrapper wrapper => EnzymeM wrapper String
debug = E.debug =<< ask

-- | Logs a string representing the DOM tree of the current element(s).
trace :: forall wrapper. DebugWarning => Wrapper wrapper => EnzymeM wrapper Unit
trace = log =<< debug
...
-- | for more info.
exists :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper Boolean
...
-- | more info.
find :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper ChildWrapper
find selector = E.find selector =<< ask

-- | Returns parent of the current element. See
-- | https://enzymejs.github.io/enzyme/docs/api/ReactWrapper/find.html for
-- | more info.
parent :: forall wrapper. Wrapper wrapper => EnzymeM ChildWrapper wrapper
parent = E.parent =<< ask
...
-- | found.
findSingle :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper ChildWrapper
...
-- | for more info.
is :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper Boolean
...
-- | for more info.
prop :: forall a wrapper. Wrapper wrapper => String -> EnzymeM wrapper a
...
-- | question is using.
unsafeSetState :: forall state wrapper. Wrapper wrapper => state -> EnzymeM wrapper Unit
unsafeSetState newState =
  E.unsafeSetState newState =<< ask

-- | A convenience function for calling `simulate'` without an `event` arg.
simulate :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper Unit
simulate eventType =
...
simulate' :: forall r wrapper. Wrapper wrapper => String -> Record r -> EnzymeM wrapper Unit
...
simulateCustom' :: forall a wrapper. Wrapper wrapper => String -> a -> EnzymeM wrapper Unit
...
clickOn :: forall wrapper. Wrapper wrapper => String -> EnzymeM wrapper Unit
...
-- | more info.
update :: EnzymeM RootWrapper Unit
update = E.update =<< ask

-- | Takes a `ChildWrapper` and runs an `EnzymeM` computation with the given
...
-- | ```
withElement :: forall a wrapper. Wrapper wrapper => ChildWrapper -> EnzymeM ChildWrapper a -> EnzymeM wrapper a
withElement wrapper =
  withReaderT $ const wrapper

-- | A version of `withElement` that takes the `ChildWrapper` wrapped in
-- | `EnzymeM` rather than “naked.” Aliased as the `>>` operator, this allows
-- | for handy chaining of operations that return a `ChildWrapper`, for
-- | example:
-- |
-- | ```purescript
-- | find ".foo" >> at 1 >> find ".bar" >> simulate "click"
-- | ```
withElementM :: forall a wrapper. Wrapper wrapper => EnzymeM wrapper ChildWrapper -> EnzymeM ChildWrapper a -> EnzymeM wrapper a
...
-- | current `Wrapper` performs the given action and returns all results of that
-- | action as an array.
-- |
-- | Example:
-- |
...
-- |
mapEach :: forall a wrapper. Wrapper wrapper => EnzymeM ChildWrapper a -> EnzymeM wrapper (Array a)
mapEach f = toArray >>= traverse \e -> withElement e f

-- | Returns all elements contained in the current `Wrapper` as an array.
toArray :: forall wrapper. Wrapper wrapper => EnzymeM wrapper (Array ChildWrapper)
...
-- |
forEach :: forall wrapper. Wrapper wrapper => EnzymeM ChildWrapper Unit -> EnzymeM wrapper Unit
forEach f = E.forEach (\e -> withElement e f) =<< ask

-- | Returns number of elements in the current context
length :: forall wrapper. Wrapper wrapper => EnzymeM wrapper Int
length = E.length <$> ask
...
-- |   prop "disabled" >>= shouldEqual true
-- | ```
withSelector :: forall a wrapper. Wrapper wrapper => String -> EnzymeM ChildWrapper a -> EnzymeM wrapper a
...
waitWhile :: EnzymeM RootWrapper Boolean -> EnzymeM RootWrapper Unit
waitWhile = waitWhile' (Milliseconds 1000.0)

-- | Performs active wait while the given condition is false. Times out with a
-- | crash after a second.
waitUntil :: EnzymeM RootWrapper Boolean -> EnzymeM RootWrapper Unit
waitUntil = waitUntil' (Milliseconds 1000.0)

-- | Performs active wait while the given condition is true. Times out with a
-- | crash after given time period.
waitWhile' :: Milliseconds -> EnzymeM RootWrapper Boolean -> EnzymeM RootWrapper Unit
waitWhile' timeout f = waitUntil' timeout $ not <$> f

-- | Performs active wait while the given condition is true. Times out with a
-- | crash after given time period.
waitUntil' :: Milliseconds -> EnzymeM RootWrapper Boolean -> EnzymeM RootWrapper Unit
